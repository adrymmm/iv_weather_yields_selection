...
**Causal estimate:** The preferred double LASSO estimate gives $\beta = -0.865$ (SE = 0.446, $p = 0.053$), implying a 1% increase in current-year yield reduces planted area the following year by approximately 0.87%, conditional on state and year fixed effects. The naive post-selection IV estimate is $\beta = -1.039$ (SE = 0.559, $p = 0.063$). Both specifications agree in direction, magnitude, and significance level. The negative supply response is consistent with rational farmer adjustment: a positive yield shock lowers expected scarcity, reducing the incentive to expand plantings.
